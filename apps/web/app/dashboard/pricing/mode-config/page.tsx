...
import { CoinsIcon, TruckIcon, Train01Icon, Airplane01Icon, SaveIcon, CheckmarkCircle02Icon } from "@hugeicons/core-free-icons"
...
export default function ModePricingConfigPage() {
  const [activeMode, setActiveMode] = React.useState("ROAD")
  const [config, setConfig] = React.useState<ModeConfig | null>(null)
  ...
  React.useEffect(() => { loadConfig(activeMode) }, [activeMode])
  ...
  const activeRulesCount = config?.rules?.length || 0
    ...
    <DashboardLayout breadcrumbs={[{ label: "Dashboard", href: "/dashboard" }, { label: "Pricing", href: "/dashboard/pricing" }, { label: "Mode Pricing" }]}>
      <div className="flex flex-col gap-6 p-4 lg:p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard label="Base Rate" value={formatMoney(Number(form.baseRate || 0), undefined, { compact: true })} icon={CoinsIcon} hint={activeModeData.label} />
          <MetricCard label="Per Kg Rate" value={formatMoney(Number(form.perKgRate || 0), undefined, { compact: true })} icon={CoinsIcon} hint={activeModeData.label} />
          <MetricCard label="Insurance" value={`${form.insuranceRate || 0}%`} icon={CheckmarkCircle02Icon} hint="Of declared value" />
          <MetricCard label="Tax (VAT)" value={`${form.taxRate || 0}%`} icon={CoinsIcon} hint="Applied to subtotal" />
        </div>
